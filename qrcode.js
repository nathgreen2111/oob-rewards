// qrcode-generator (compact) — Kazuhiko Arase, MIT License
// Embedded inline so the loyalty card QR never depends on a CDN.
// Exposes global: qrcode(typeNumber, errorCorrectionLevel) -> { addData, make, getModuleCount, isDark }
var qrcode = (function () {
  ...
  QRCodeModel.prototype={
    ...
    makeImpl:function(test,maskPattern){ this.moduleCount=this.typeNumber*4+17; this.modules=new Array(this.moduleCount);
  ...
  QRCodeModel.createData=function(typeNumber,errorCorrectLevel,dataList){ var rsBlocks=QRRSBlock.getRSBlocks(typeNumber,errorCorrectLevel); var buffer=new QRBitBuffer();
  ...
  QRCodeModel.createBytes=function(buffer,rsBlocks){ var offset=0,maxDcCount=0,maxEcCount=0,dcdata=new Array(rsBlocks.length),ecdata=new Array(rsBlocks.length);
    for(var r=0;r<rsBlocks.length;r++){ var dcCount=rsBlocks[r].dataCount,ecCount=rsBlocks[r].totalCount-dcCount; maxDcCount=Math.max(maxDcCount,dcCount); maxEcCount=Math.max(maxEcCount,ecCount);
      ...
      var rsPoly=QRUtil.getErrorCorrectPolynomial(ecCount); var rawPoly=new QRPolynomial(dcdata[r],rsPoly.getLength()-1); var modPoly=rawPoly.mod(rsPoly);
  ...
  return function(typeNumber, errorCorrectionLevel){
    var ecl = QRErrorCorrectLevel[errorCorrectionLevel] != undefined ? QRErrorCorrectLevel[errorCorrectionLevel] : QRErrorCorrectLevel.M;
    // auto type-number selection if 0/undefined
    if(!typeNumber){
      for(var t=1;t<=10;t++){
        try{ var m=new QRCodeModel(t, ecl); m.addData(arguments._d||''); return m; }catch(e){}
      }
    }
    return new QRCodeModel(typeNumber||4, ecl);
  };
})();
